(function () {
  ...
  function renderBasics() {
    ...
    grid.appendChild(
      createCheckboxField("Enabled", state.enabled, function (value) {
        state.enabled = value;
        render();
      }),
    );

    replaceChildren(dom.basics, grid);
  }
  ...
  function renderConditions() {
    ...
    replaceChildren(dom.conditions, root);
  }
  ...
  function renderActivation() {
    ...
    replaceChildren(dom.activation, grid);
  }
  ...
  function renderAction() {
    ...
    const actionTypeField = createSelectField(
      "Action type",
      manifest.actions.map(function (item) {
    ...
    card.className = "card";
    card.appendChild(
      createMetaHeader(actionMeta.label, actionMeta.description),
    );
    card.appendChild(
      createFieldsBlock(actionMeta.fields, state.action, function (key, value) {
        state.action[key] = value;
        render();
      }),
    );
    replaceChildren(dom.action, actionTypeField, card);
  }
  ...
  function renderOutput() {
    ...
    dom.status.className = validation.isValid
      ? "status"
      : "status status--error";
  ...
  function renderGroup(group, path) {
    ...
    header.appendChild(
      createMetaHeader(
        path.length === 0 ? "Root group" : "Condition group",
    ...
    addGroupButton.className = "button--secondary";
  ...
  function renderCondition(condition, path) {
    ...
    const toolbar = document.createElement("div");
    toolbar.className = "toolbar toolbar--leaf";
    const removeButton = document.createElement("button");
    removeButton.type = "button";
    removeButton.className = "button--secondary";
  ...
  function createAddConditionControl(group) {
    ...
    const select = createSelect(
      manifest.conditions.map(function (item) {
        return { value: item.typeId, label: item.label };
      }),
      manifest.defaultConditionType,
      function () {},
    );
    ...
    addButton.addEventListener("click", function () {
      group.children.push(
        createDefinitionFromManifest(conditionMap[select.value]),
      );
      render();
    });
  ...
  function createRuleState() {
    return {
      ...
      action: createDefinitionFromManifest(
        actionMap[manifest.defaultActionType],
      ),
    };
  }
  ...
  function removeNode(path) {
    ...
    const parent =
      parentPath.length === 0 ? state.rootGroup : getNode(parentPath);
