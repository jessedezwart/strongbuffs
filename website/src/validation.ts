...
function isConditionGroup(node: ConditionNode): node is ConditionGroup {
  return (
    node.type === "group" && Array.isArray((node as ConditionGroup).children)
  );
}

export function isManifestFieldValid(
  field: ManifestField,
  value: unknown,
): boolean {
  if (field.kind === "text") {
...
export function validateDefinition(
  ...
  meta:
    | ManifestRefs["conditionMap"][string]
    | ManifestRefs["actionMap"][string]
    | undefined,
  errors: string[],
): void {
  if (!definition || !meta) {
...
export function validateRule(
  ...
  validateDefinition(
    rule.action,
    manifestRefs.actionMap[rule.action.type],
    errors,
  );
